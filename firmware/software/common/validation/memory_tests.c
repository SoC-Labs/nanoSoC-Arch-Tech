...
#define  ROM_BASE_ADDR   0x10000000
...
int main (void)
{
  ...
  puts(" - Simple memory test - revision $Revision: 371321 $\n");
...
int apb_io_test(void)
{
  ...
  if (hardfault_occurred!=0) err_code |= 1<<0;
  // temp_data = address_test_read(APB_IO_BASE_ADDR+APB_IO_SIZE-4); - APB Slave Error if No DMA in System only test lower limit
  // if (hardfault_occurred!=0) err_code |= 1<<1; - Tested in APB Mux Test Anyway
