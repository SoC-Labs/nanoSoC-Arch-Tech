...
module nanosoc_axi_stream_io_8_txd_from_file #(
  ...
  reg [1023:0]   code_filename;  // String to hold the actual filename to use

  initial begin
    valid <= 0;
    flen = 0;
    
    // Check for plusarg, use parameter as default if not provided
    if (!$value$plusargs("CODEFILENAME=%s", code_filename)) begin
      code_filename = CODEFILENAME;
    end
    
    fd = $fopen(code_filename, "r");
    ...
    if (FAST_LOAD == 0) begin
      ...
      $readmemh(code_filename, adpbuf, 20);
